...
6. 【买点评分卡·强制】输出任何「买入/加仓」建议前，必须附 5 维打分：①估值锚（低估值或目标空间足）②回调日（非红日追高，回调≥2日）③恐慌强度（非恐慌后首日反弹）④资金确认（DDX/主力净流入转正或连正）⑤试探仓位（≤300 或额度内试探）。每 ✓ 记 1 分；**总分 <3 分 → 必须标注「追高型买入 · 按打分卡放弃」，不输出买入指令**；≥3 分才可建议并附得分。卖出/减仓/持有/等待建议不需打分。"""
