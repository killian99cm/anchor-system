...
# 版本号从数据合同动态读取，避免每次重建把过期的硬编码徽标写进 HTML
version = (data.get("system_version") or "").strip()
...
<title>Anchor __VERSION__ · 私有投资驾驶舱</title>
<style>
...
.grid3{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-bottom:15px}.grid2{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:15px}.list{max-height:330px;overflow:auto}.rule-row,.risk-row,.watch-row,.action-row{padding:9px 0;border-bottom:1px solid rgba(154,188,225,.07);font-size:10.5px}.rule-row:last-child,.risk-row:last-child,.watch-row:last-child,.action-row:last-child{border-bottom:0}.rule-row{display:flex;gap:8px;align-items:flex-start}.dot{width:8px;height:8px;margin-top:4px;flex:0 0 auto;border-radius:50%;background:var(--green)}.dot.watch{background:var(--amber);box-shadow:0 0 7px rgba(250,178,25,.45)}.dot.action{background:var(--red);box-shadow:0 0 9px rgba(230,103,103,.52)}.muted{color:var(--muted)}.risk-row,.watch-row{display:grid;grid-template-columns:25px 1fr auto;gap:8px;align-items:center}.risk-level{font-size:13px}.risk-detail,.watch-trigger{color:var(--dim);font-size:9px;text-align:right}.watch-today{grid-column:2/4;font-size:9px;line-height:1.45;margin-top:3px;color:var(--muted);overflow-wrap:anywhere}.watch-today.ok{color:var(--green)}.watch-today.bad{color:var(--red)}.watch-today.warn{color:var(--amber)}.action-row strong{display:block;color:#fff;font-size:10px}.action-row span{display:block;color:var(--muted);margin-top:2px}.action-row small{display:block;color:var(--dim);font-size:9px;margin-top:3px}.empty{padding:16px 0;color:var(--dim);font-size:10px}
...
<body>
<div class="app">
<header class="topbar"><div class="brand"><span class="brand-mark">⚓</span><span><em>Anchor</em> Terminal</span></div><span class="badge">__VERSION__ PRIVATE</span><span class="status safe" id="headerStatus">状态计算中</span><span class="spacer"></span><div class="topstat">数据源<b>本地 portfolio_data.json</b></div><div class="topstat">更新<b id="updateLabel">--</b></div><div class="clock" id="clock">--</div></header>
...
<div class="meta">Anchor __VERSION__ · 四层占比由统一数据合同生成 · 真实数据只存本地<br>回撤基准、风险状态、冻结状态、操作计数均来自 data_processor.process_all()</div>
<footer class="footer"><a href="Anchor/08-website/anchor-pro.html" data-anchor-link="08-website/anchor-pro.html">📖 体系总览</a> · <a href="Anchor/05-scripts/anchor_calculator.html" data-anchor-link="05-scripts/anchor_calculator.html">🔢 仓位计算器</a> · <a href="Anchor/08-website/diagrams/pyramid-4layer.html" data-diagram-link="pyramid-4layer.html">▣ 体系图集</a><br>图集：<a href="Anchor/08-website/diagrams/pyramid-4layer.html" data-diagram-link="pyramid-4layer.html">四层金字塔</a> · <a href="Anchor/08-website/diagrams/data-pipeline.html" data-diagram-link="data-pipeline.html">数据管道</a> · <a href="Anchor/08-website/diagrams/decision-loop.html" data-diagram-link="decision-loop.html">决策闭环</a> · <a href="Anchor/08-website/diagrams/architecture.html" data-diagram-link="architecture.html">系统架构</a><br>投资有风险，决策需谨慎</footer>
</div>
<script>
var D=__DATA__;
...
(function(){var root=document.getElementById("kpi"),ops=D.ops_state||{},dd=D.drawdown_state||{},hc=D.holding_counts||{},cashRatio=Number(D.portfolio_state?D.portfolio_state.cash_ratio:0)*100,items=[
  ["Total assets","¥"+money(D.total),"基金 ¥"+money(D.fundMv)+" · 股票 ¥"+money(D.stockMv)+" · 自有 ¥"+money(dd.net_assets),"var(--blue)"],
  ["Hold PnL",signed(D.totalPnl),hc.active_label||"持仓动态计算",Number(D.totalPnl||0)>=0?"var(--red)":"var(--green)"], /* 红盈绿亏 */
  ["Drawdown",(Number(dd.dd_pct||D.dd_pct||0)>=0?"+":"")+Number(dd.dd_pct||D.dd_pct||0).toFixed(1)+"%","高点 ¥"+money(dd.peak_assets||D.peak_assets)+" · 净值 ¥"+money(dd.net_assets),levelClass(dd.dd_level)==="safe"?"var(--green)":"var(--amber)"],
...
(function(){ /* 四层动态金字塔 · 底部压舱石 → 顶部现金 */
  var root=document.getElementById("pyramidSVG");if(!root)return;
  var order=["bedrock","core","sat","cash"],lookup={},rows=D.layers||[];
  rows.forEach(function(l){lookup[l.key]=l});
  var items=order.map(function(k){return lookup[k]}).filter(Boolean);
  if(!items.length)return;
  var W=420,H=240,pad=46,sum=items.reduce(function(a,l){return a+Number(l.target||0)},0)||1;
  var svg=svgNode("svg",{viewBox:"0 0 "+W+" "+H,class:"pyramid-svg",role:"img","aria-label":"四层金字塔资产配置"});
  var base=40,tip=H-46,step=(tip-base)/(items.length);
  items.forEach(function(l,i){
    var color={bedrock:"#3987e5",core:"#36d39c",sat:"#fab219",cash:"#9085e9"}[l.key]||"#3987e5";
    var y1=tip-step*i, y2=tip-step*(i+1)+1;
    var t=Number(l.target||0),ratio=t/sum,minHalf=20,halfW=Math.max(minHalf,W/2-90+ratio*(W/2-100));
    var x1=W/2-halfW, x2=W/2+halfW;
    var poly=svgNode("polygon",{points:(W/2)+","+y1+" "+x2+","+(y1+step*0.22)+" "+x2+","+y2+" "+x1+","+y2+" "+x1+","+(y1+step*0.22),fill:color,fillOpacity:0.16,stroke:color,strokeWidth:1.2});
    svg.appendChild(poly);
    var cx=W/2, cy=y1+step*0.55;
    var name=svgNode("text",{x:cx,y:cy-2,class:"pyramid-layer-name"});name.textContent=l.label;
    var sub=svgNode("text",{x:cx,y:cy+13,class:"pyramid-layer-sub"});sub.textContent=Number(l.pct||0).toFixed(1)+"% / 目标 "+l.target+"%";
    svg.appendChild(name);svg.appendChild(sub);
  });
...
(function(){var root=document.getElementById("watchlist"),wl=D.wl||[];document.getElementById("watchCount").textContent=wl.length+" 项";if(!wl.length){add(root,"div","empty","暂无关注项");return}wl.forEach(function(w){var row=node("div","watch-row");add(row,"span","risk-level",w.status&&w.status.indexOf("🔴")>=0?"🔴":"🟡");var main=add(row,"span",null,w.sector||w.s||"未命名板块");add(main,"small","muted",(w.etf_code||w.c||"")+" · "+(w.status||w.st||""));add(row,"span","watch-trigger","触发："+(w.trigger||w.t||"观察"));if(w.today){var tm=w.today_meta||{};var lv=(tm.verdict||"").indexOf("🟢")>=0?"ok":((tm.verdict||"").indexOf("⛔")>=0?"bad":"warn");var t=add(row,"span","watch-today",w.today);t.title=(tm.a2_detail||"")+"\n条件②：收盘 "+(tm.close==null?"—":tm.close)+" vs MA"+(tm.ma_period||5)+" "+(tm.ma==null?"—":tm.ma)+"\n数据时点 "+(tm.computed_at||"")+" · 收盘已定格="+(tm.close_confirmed===true)+((tm.missing&&tm.missing.length)?"\n缺口："+tm.missing.join("；"):"")+((tm.caveats&&tm.caveats.length)?"\n⚠️须裁决："+tm.caveats.join("；"):"");t.className="watch-today "+lv}root.appendChild(row)})})();
...
(function(){
  ...
  // 体系图集链接：diagrams 在 Anchor/08-website/diagrams/
  document.querySelectorAll("[data-diagram-link]").forEach(function(a){
    a.href=anchorBase()+"08-website/diagrams/"+a.getAttribute("data-diagram-link");
  });
})();
</script>
</body>
...
html = html.replace("__DATA__", json.dumps(embed, ensure_ascii=False).replace("</", "<\\/").replace("<!--", "<\\!--")).replace("__VERSION__", version)  # 8/17 审计：防 </script> 与 <!-- 破坏内嵌脚本
