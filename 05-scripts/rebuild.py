...
(function(){var root=document.getElementById("trendGrid"),data=D.chart||[];renderTrendCard(root,data,"sh","上证指数","#fab219",function(v){return Number(v||0).toFixed(0)});renderTrendCard(root,data,"st","科创50","#3987e5",function(v){return Number(v||0).toFixed(0)});renderTrendCard(root,data,"pnl","日盈亏","#36d39c",function(v){return (Number(v||0)>=0?"+":"")+Number(v||0).toFixed(0)})})();
