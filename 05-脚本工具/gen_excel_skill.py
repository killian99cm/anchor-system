...
auto=[("示例黄金基金","每天 ¥15",450,"避险","雷打不动",next((h['mv'] for h in sorted_H if'黄金'in h['name']and'国泰'in h['name']),0)),
      ("华泰纳斯达克100 A","每天 ¥10(上限)",300,"QDII限额","不能停",next((h['mv'] for h in sorted_H if'华泰'in h['name']and'纳'in h['name']),0)),
      ("示例指数基金全球高端制造C","每天 ¥20(动态)",600,"QDII限额","回调5%→¥30,10%→¥50+单笔",next((h['mv'] for h in sorted_H if'高端制造'in h['name']),0))]
...
man=[("上证红利联接A","每月初手动","¥500",next((h['mv'] for h in sorted_H if'红利'in h['name']),0),"—","进行中"),
     ...
     ("示例联接基金证券ETF联接C","7月10日前","¥500-1,000",next((h['mv'] for h in sorted_H if'证券'in h['name']),0),"2026-07-10","待执行")]
