...
try:
    ...
    # 🔴 2026-09-25 订正（本测试当时是**结构性恒红的 stale 测试**）：
    #    0.6 步早已是 `soft=True`（sync_all 注释：2026-09-16 深夜加）⇒ 失败时输出的是
    #    `0.6.取数候选 [GAP]（取数缺口，非链故障…）`，**根本不会出现** `[FAILED` 与
    #    「非致命，继续」两串（那是 soft=False 分支的措辞）⇒ 原断言**永远不可能通过**。
    #    ⚠️ 而它**未接入 smoke** ⇒ 长期无人发现（正是「门禁自身带病」家族）。
    #    本次只把断言对齐到 soft-gap 语义，**判据强度不变**（仍要求「0.6 失败但链路继续」）。
    after_06 = out.split("0.6.取数候选 [GAP", 1)
    ...
    checks = [
        ...
        ("0.6 判为【非致命·继续】（soft-gap：缺口不入失败汇总）",
         "[GAP]" in out and "取数缺口，非链故障" in out),
