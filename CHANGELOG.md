...
**修改**：`05-scripts/data_auto_fill.py`（v3 重写；**§⑤ 补丁 A/B** 加 `_classify_failure()` + 复用当日读数 + `grade` 不降级）、`data_pipeline.py`（`--coverage` + A-2 断言）、`smoke_test.py`（新增 `[7-c2c]`）、`.gitignore`（+`data_source_health.json`）、`sync_all.py`（**§⑤ 补丁 C** `step(soft=True)` —— **不入库**，见下行）
