...
MX_TOL_PP = 1.00   # 容忍带（百分点）：两个口径在此带宽内可能给出**相反结论** ⇒ fail-closed
#
# 🔴 定标依据（2026-09-24 · **首次对拉实测** · `compare_mx_push2.py`）
#    裁定 #C1-20（9/23）写下本带时**无同日可比值**（push2 全族限流）⇒ `0.15` 是**估的**。
#    push2 恢复后首次两级皆备，实测（2026-09-23，n=4 板块）：
#      · |Δpp|（mx − push2）：**max 0.8451 ／ mean 0.3813**，且 **Δ 符号 4/4 全为负**
#        （不像噪声，像**系统性口径偏差**，成因未查明）
#      · **1 条判定翻转**：人形机器人（BK1184）mx −0.2593 ⇒ **旧带下判「不成立（完全）」**，
#        push2 +0.30 ⇒ 判「**命中**」（分支②）。**方向是危险的那一侧** —— 旧带放行了本该禁买的。
#    ⇒ 本带的**书面目的**（防两口径相反结论）在 `0.15` 下**未被实现** ⇒ 按实测 max|Δ| 上取整
#      并留余量取 **1.00pp**。⛔ 不得再取小于实测 max|Δ| 的值。
#
# ⚠️ **本值＝临时定标**（n=1 个交易日）：每次 `compare_mx_push2.py` 出现新的成对交易日，
#    须重算 max|Δ| 并复核；**成对样本 ≥5 个交易日**后定稿。带变宽＝更 fail-closed（收紧），
#    故不构成放宽；但**取到远小于实测 max|Δ| 的值＝放宽**，须先问用户。
