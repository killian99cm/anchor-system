...
H.forEach(function(h){var mv=h.mv||0,n=h.name||'',g=h.group||'';if(mv<1||g==='已清仓'||g==='待结算')return;if(n.indexOf('余额宝')!==-1){caMv=mv;return;}if(g==='全局固收'||n.indexOf('黄金')!==-1||n.indexOf('债券')!==-1){bMv+=mv;bF.push(h);}else if(n.indexOf('纳斯达克')!==-1||n.indexOf('纳指')!==-1||n.indexOf('天弘通利')!==-1){cMv+=mv;cF.push(h);}else{sMv+=mv;sF.push(h);}});
