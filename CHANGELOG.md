...
- 🔬 **深度分析**（5 代理交叉归因 + 盈亏口径核实）：创新药 2025-08-19 建仓至 2026-08-21 全周期复盘 → `04-reviews/special/2026-08-21-创新药操作深度分析.md`
  - **核心结论**：12 个月累计净亏 **-248.04**（约 -10.3% 净投入口径），当前 +2.27% 浮盈全靠 8/20 单日行情
  - **纠正此前推算**：回填时按 App 买入口径 7,224.46 推出「+171.96 盈利」为**低成本假象**——平台 `cumul=-248.04` 反推买入口径为 7,644.46；真实拆解 = 已实现 ≈-305.28 + 浮盈 +57.24
  - **行为模式**：追涨/右侧惯性为主风险；「摊低→割肉」循环；高抛低吸实为亏损成交；唯一纪律 = 小额定投 + 底部回补
- 📝 **decision_log 准确化**：#14 review_note 补整仓累计口径（7/21 位置正确 ≠ 整仓操作正确）；#1/#14 补 snapshot（8/20 HSSCID 3104.56 +5.36%、ETF 获利了结 2.84 亿；7/21 资金切换背景）——统计不变（80% / -6.23% / 0.93:1，加仓 50%）
- ✅ **买入口径澄清**（用户 8/21 补充）：9/25 实为**卖出 293.77**（非买入）；12/08 转换转入成本基数确认 = **208.87 份 × 净值 1.0986 = 229.46**（差 0.0046 舍入）；三口径分解：明细 7,594.46（纯买 6,950 + 定投 415 + 转换 229.46）、平台 7,644.46 = 明细 + 50、App 7,224.46 = 明细 - 370；**剩余 ~45-50 元来源未定**（C 类无申购费，用户 App 无可查字段，疑分红再投资/转换费/未导出流水，如实保持未定）；对外口径统一用平台 `cumul=-248.04`
